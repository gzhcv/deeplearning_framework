...
__global__ void matrix_elementwise_add_by_const_kernel(const float *d_in,
...
int DLGpuMatrixElementwiseAddByConst(const DLArrayHandle input, float val,
    ...
    int threads_per_block = 1024;
    int num_blocks = (n + threads_per_block - 1) / threads_per_block;
    matrix_elementwise_add_by_const_kernel << < num_blocks, threads_per_block >> > (
            input_data, output_data, val, n);
    return 0;
}

__global__
void matrix_elementwise_subtract_by_const_kernel(const float *d_in,
                                                 float *d_out, float val, int n) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < n) {
        d_out[index] = d_in[index] - val;
...
int DLGpuMatrixElementwiseSubtractByConst(const DLArrayHandle input, float val,
    ...
    int threads_per_block = 1024;
    int num_blocks = (n + threads_per_block - 1) / threads_per_block;
    matrix_elementwise_subtract_by_const_kernel << < num_blocks, threads_per_block >> > (
            input_data, output_data, val, n);
    return 0;
}

__global__ void matrix_elementwise_div_by_const_kernel(const float *d_in,
                                                       float *d_out, float val, int n) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < n) {
        d_out[index] = d_in[index] / val;
...
int DLGpuMatrixElementwiseDivByConst(const DLArrayHandle input, float val,
    ...
    int threads_per_block = 1024;
    int num_blocks = (n + threads_per_block - 1) / threads_per_block;
    matrix_elementwise_div_by_const_kernel << < num_blocks, threads_per_block >> > (
            input_data, output_data, val, n);
    return 0;
}
